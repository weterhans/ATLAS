...
<div class="p-6 md:p-8">
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center">
            ...
            <h1 class="text-2xl font-bold text-gray-800 ml-4">Save Data CNSD</h1>
        </div>
    </div>
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <ul class="divide-y divide-gray-200">
                ...
                <li class="px-6 py-4 flex items-center justify-between hover:bg-gray-50 cursor-pointer"
                    x-data="{ reportType: '{{ $item->type }}' }" {{-- Tambah reportType di sini --}}
                    ...
                    <div class="ml-4 flex-shrink-0 flex items-center space-x-2">
                        ...
                        {{-- [PERUBAHAN] Hapus x-data & x-show, ubah x-on:click.stop --}}
...
{{-- >>> AWAL MODAL TAMBAH SAVE DATA CNSD <<< --}}
{{-- ... (Modal Tambah tidak berubah) ... --}}
<div id="add-savedata-modal" x-data="{ showModal: false, optionType: 'Harian' }" x-show="showModal" x-on:open-modal.window="showModal = true; optionType = 'Harian'; setTimeout(() => { document.getElementById('add-savedata-form').reset(); document.getElementById('save_tanggal').value = '{{ date('Y-m-d') }}'; document.querySelector('input[name=print][value=YA]').checked = true; }, 50);" x-on:keydown.escape.window="showModal = false" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center z-50 px-4" style="display: none;" aria-labelledby="modal-title-cnsd-savedata" role="dialog" aria-modal="true">
    <div class="relative mx-auto p-6 border w-full max-w-lg shadow-lg rounded-xl bg-white" @click.outside="showModal = false">
        <div class="flex justify-between items-center border-b pb-3 mb-4">
            <h3 id="modal-title-cnsd-savedata" class="text-xl font-semibold text-gray-900">Tambah Data</h3>
        ...
        <form id="add-savedata-form" action="{{ route('cnsd.save_data.store') }}" method="POST" class="space-y-4">
            @csrf <input type="hidden" name="grup" value="CNSD"> <input type="hidden" name="type" x-bind:value="optionType">
            ...
            <div> <label class="block text-base font-medium text-gray-700">GROUP</label> <input type="text" value="CNSD" readonly class="mt-1 block w-full bg-gray-100 border-gray-300 rounded-lg shadow-sm p-2 text-gray-500 cursor-not-allowed text-base"> </div>
...
{{-- >>> [BARU] AWAL MODAL DETAIL KEGIATAN (HARIAN) <<< --}}
{{-- ... (Modal Kegiatan Harian tidak berubah) ... --}}
<div id="detail-kegiatan-modal" x-data="{ showModal: false, report: { dinas: '', tanggal_db: '' }, activity: null, loadingActivity: false, activityError: '' }" x-show="showModal" x-on:open-kegiatan-modal.window=" showModal = true; report = $event.detail.report; activity = null; activityError = ''; if (report.dinas && report.tanggal_db) { loadingActivity = true; if(typeof window.fetchActivityDetails === 'function') { window.fetchActivityDetails($data, report.tanggal_db, report.dinas); } else { console.error('Fungsi fetchActivityDetails tidak ditemukan.'); activityError = 'Error: Fungsi internal tidak siap.'; loadingActivity = false; } } else { activityError = 'Data laporan tidak lengkap untuk mencari kegiatan.'; loadingActivity = false; } " x-on:keydown.escape.window="showModal = false" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center z-50 px-4" style="display: none;" aria-labelledby="modal-title-cnsd-kegiatan" role="dialog" aria-modal="true">
    <div class="relative mx-auto p-6 border w-full max-w-lg shadow-lg rounded-xl bg-white" @click.outside="showModal = false">
        <div class="flex justify-between items-center border-b pb-3 mb-4">
            <h3 id="modal-title-cnsd-kegiatan" class="text-xl font-semibold text-gray-900">Detail Kegiatan</h3>
            <button @click="showModal = false" type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-base p-1.5 ml-auto inline-flex items-center" aria-label="Close modal">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                </svg>
...
{{-- >>> [BARU] AWAL MODAL EDIT SAVE DATA CNSD <<< --}}
{{-- ... (Modal Edit tidak berubah) ... --}}
<div id="edit-savedata-modal" x-data="{ showModal: false, optionType: 'Harian', reportId: null }" x-show="showModal" x-on:open-edit-modal.window=" showModal = true; const report = $event.detail.report; reportId = report.id; optionType = report.type; const form = document.getElementById('edit-savedata-form'); form.action = `{{ url('cnsd/save-data') }}/${report.id}`; form.querySelector('[name=type]').value = report.type; form.querySelector('[name=tanggal]').value = report.tanggal; form.querySelector(`input[name=print][value='${report.print}']`).checked = true; if(report.type === 'Harian') { if(report.dinas) form.querySelector(`input[name=dinas][value='${report.dinas}']`).checked = true; form.querySelector('[name=mantek]').value = report.mantek; } if(report.type === 'Bulanan') { form.querySelector('[name=nama_alat]').value = report.nama_alat; form.querySelector('[name=sampai]').value = report.sampai; } window.updateRadioButtonsUI(form); " x-on:keydown.escape.window="showModal = false" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center z-50 px-4" style="display: none;" aria-labelledby="modal-title-cnsd-edit" role="dialog" aria-modal="true">
    <div class="relative mx-auto p-6 border w-full max-w-lg shadow-lg rounded-xl bg-white" @click.outside="showModal = false">
        <div class="flex justify-between items-center border-b pb-3 mb-4">
            <h3 id="modal-title-cnsd-edit" class="text-xl font-semibold text-gray-900">Edit Data</h3>
            <button @click="showModal = false" type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-base p-1.5 ml-auto inline-flex items-center" aria-label="Close modal">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                </svg>
        ...
        <form id="edit-savedata-form" action="" method="POST" class="space-y-4">
            @csrf @method('PUT') <input type="hidden" name="grup" value="CNSD"> <input type="hidden" name="type" x-bind:value="optionType">
            <div> <label class="block text-base font-medium text-gray-700 mb-1">OPSI</label>
                <div class="flex rounded-lg border border-gray-300 overflow-hidden"> <button type="button" @click="optionType = 'Harian'" :class="{ 'bg-indigo-600 text-white': optionType === 'Harian', 'bg-gray-100 text-gray-600 hover:bg-gray-200': optionType !== 'Harian' }" class="flex-1 text-center p-2 rounded-l-md transition-colors text-base font-medium">HARIAN</button> <button type="button" @click="optionType = 'Bulanan'" :class="{ 'bg-indigo-600 text-white': optionType === 'Bulanan', 'bg-gray-100 text-gray-600 hover:bg-gray-200': optionType !== 'Bulanan' }" class="flex-1 text-center p-2 border-l border-gray-300 rounded-r-md transition-colors text-base font-medium">BULANAN</button> </div>
            </div>
...
{{-- >>> [BARU] AWAL MODAL KONFIRMASI DELETE <<< --}}
{{-- ... (Modal Delete tidak berubah) ... --}}
<div id="delete-savedata-modal" x-data="{ showModal: false, itemName: '', formAction: '' }" x-show="showModal" x-on:open-delete-modal.window=" showModal = true; itemName = $event.detail.name; formAction = `{{ url('cnsd/save-data') }}/${$event.detail.id}`; document.getElementById('delete-savedata-form').action = formAction; " x-on:keydown.escape.window="showModal = false" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center z-50 px-4" style="display: none;" aria-labelledby="modal-title-cnsd-delete" role="dialog" aria-modal="true">
    <div class="relative mx-auto p-6 border w-full max-w-lg shadow-lg rounded-xl bg-white" @click.outside="showModal = false">
        <div class="flex justify-between items-center border-b pb-3 mb-4">
            <h3 id="modal-title-cnsd-delete" class="text-xl font-semibold text-gray-900">Konfirmasi Hapus Data</h3>
            <button @click="showModal = false" type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-base p-1.5 ml-auto inline-flex items-center" aria-label="Close modal">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                </svg>
...
<div id="detail-summary-modal" x-data="{ showModal: false, report: { type: '', tanggal: '', tanggal_formatted: '', dinas: '', mantek: '', print: '', grup: '', nama_alat: '', sampai: '', sampai_db: '', file_path: '' }, activity: null, groupedActivities: null, loadingActivity: false, activityError: '' }" x-show="showModal" x-on:open-summary-modal.window=" showModal = true; report = $event.detail.report; activity = null; groupedActivities = null; activityError = ''; loadingActivity = true; if (report.type === 'Harian' && report.tanggal && report.dinas) { if(typeof window.fetchSummaryActivityDetails === 'function') { window.fetchSummaryActivityDetails($data, report.tanggal, report.dinas); } else { console.error('Fungsi fetchSummaryActivityDetails tidak ditemukan.'); activityError = 'Error: Fungsi internal Harian tidak siap.'; loadingActivity = false; } } else if (report.type === 'Bulanan' && report.tanggal && report.sampai_db) { if(typeof window.fetchMonthlyActivityDetails === 'function') { window.fetchMonthlyActivityDetails($data, report.tanggal, report.sampai_db); } else { console.error('Fungsi fetchMonthlyActivityDetails tidak ditemukan.'); activityError = 'Error: Fungsi internal Bulanan tidak siap.'; loadingActivity = false; } } else { loadingActivity = false; } " x-on:keydown.escape.window="showModal = false" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center z-50 px-4" style="display: none;" aria-labelledby="modal-title-cnsd-summary" role="dialog" aria-modal="true">
    <div class="relative mx-auto p-6 border w-full max-w-xl shadow-lg rounded-xl bg-white" @click.outside="showModal = false">
        ...
        <div class="text-base max-h-[70vh] overflow-y-auto pr-2 space-y-4"> {{-- Tambah space-y-4 --}}

            {{-- [PERUBAHAN] Detail Laporan Utama (Field disembunyikan kondisional) --}}
            <div class="space-y-1">
                {{-- Selalu Tampil --}}
                <div class="flex">
                ...
                {{-- Hanya Tampil Jika Harian --}}
                <template x-if="report.type === 'Harian'">
                    <> {{-- Fragment untuk membungkus multiple element --}}
                        <div class="flex">
...
{{-- >>> [MODAL BARU] AWAL MODAL DETAIL JADWAL BULANAN <<< --}}
<div id="detail-jadwal-bulanan-modal"
    ...
    style="display: none;"
    aria-labelledby="modal-title-cnsd-jadwal-bulanan" role="dialog" aria-modal="true">
    ...
    <div class="relative mx-auto p-6 border w-full max-w-xl shadow-lg rounded-xl bg-white"
        ...
        {{-- Header Modal --}}
        <div class="flex justify-between items-center border-b pb-3 mb-4">
            <h3 id="modal-title-cnsd-jadwal-bulanan" class="text-xl font-semibold text-gray-900">Detail Jadwal Bulanan</h3>
            <button @click="showModal = false" type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-base p-1.5 ml-auto inline-flex items-center" aria-label="Close modal">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                </svg>
...
{{-- >>> [MODAL BARU] AKHIR MODAL DETAIL JADWAL BULANAN <<< --}}
...
@push('scripts')
<script>
    // ... (fungsi updateRadioButtonsUI tidak berubah)
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // ... (logika modal tambah & edit tidak berubah) ...
        ...
        // --- LOGIKA FETCH DATA MODAL DETAIL ---
        ...
        window.fetchScheduleDetails = async function(alpineData, tanggal, dinas) {
            ...
            try {
                const response = await fetch(`{{ route('cnsd.save_data.getSchedule') }}?tanggal=${tanggal}&dinas=${dinas}`, {
                    method: 'GET',
                    headers: {
        ...
        window.fetchActivityDetails = async function(alpineData, tanggal, dinas) {
            ...
            try {
                const response = await fetch(`{{ route('cnsd.save_data.getActivity') }}?tanggal=${tanggal}&dinas=${dinas}`, {
                    method: 'GET',
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest',
                        'Accept': 'application/json'
        ...
        window.fetchMonthlyActivityDetails = async function(alpineData, startDate, endDate) {
            ...
            try {
                const response = await fetch(`{{ route('cnsd.save_data.getRangeActivity') }}?start_date=${startDate}&end_date=${endDate}`, {
                    method: 'GET',
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest',
                        'Accept': 'application/json'
        ...
        // [FUNGSI BARU] Fetch JADWAL RENTANG TANGGAL (untuk modal Jadwal Bulanan)
        window.fetchMonthlyScheduleDetails = async function(alpineData, startDate, endDate) {
            ...
            try {
                // Panggil route baru
                const response = await fetch(`{{ route('cnsd.save_data.getRangeSchedule') }}?start_date=${startDate}&end_date=${endDate}`, {
                    method: 'GET',
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest',
                        'Accept': 'application/json'
                ...
                const data = await response.json();
                // Simpan data ke groupedSchedules
