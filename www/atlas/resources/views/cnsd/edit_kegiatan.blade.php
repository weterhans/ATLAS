...
<div class="p-6 md:p-8">
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center">
            <a href="{{ route('cnsd.kegiatan') }}" class="p-2 rounded-md hover:bg-gray-200" aria-label="Kembali ke Log Kegiatan CNSD">
                <svg class="w-6 h-6 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                </svg>
            </a>
            <h1 class="text-2xl font-bold text-gray-800 ml-4">Edit Kegiatan CNSD</h1>
        </div>
    ...
    <div class="max-w-2xl mx-auto bg-white p-6 rounded-xl shadow-lg">
        <form action="{{ route('cnsd.kegiatan.update', $activity) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="mt-2 flex flex-wrap gap-4 items-start">
                    ...
                    <div class="relative group w-24 h-24 border rounded-lg overflow-hidden existing-lampiran-item">
                        ...
                        <a href="{{ $fileUrl }}"
                            @if($isImage)
                            data-lightbox-url="{{ $fileUrl }}" {{-- Simpan URL di sini --}}
                            class="block w-full h-full cursor-pointer js-lightbox-trigger" {{-- Hapus onclick, tambah class & data- --}}
                            @else
                ...
                <p class="mt-1 text-xs text-gray-500">PNG, JPG, ... Total maks 5 file.</p>
                {{-- Error messages --}}
            ...
            <div class="flex justify-end items-center border-t pt-6 mt-6 space-x-2">
                <a href="{{ route('cnsd.kegiatan') }}" class="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300">
                    Kembali
                </a>
...
<div id="image-lightbox" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 hidden z-[60]" aria-labelledby="lightbox-title-cnsd-kegiatan-edit" role="dialog" aria-modal="true">
    <img id="lightbox-image" src="" alt="Preview Lampiran CNSD" class="max-w-full max-h-full object-contain">
    ...
    <p id="lightbox-title-cnsd-kegiatan-edit" class="sr-only">Image Preview</p>
</div>
@endsection
